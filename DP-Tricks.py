...
# palindrome partition problem memoized version but not the most optimal soln
# qs asks min num of partition to make a string palindrome
...
def palin_part(arr):
    ...
    for k in range(i,j):
        ...
        temp = 1 + palin_part(arr[i:k+1]) + palin_part(arr[k+1:j+1])
        if temp < minim:
            minim = temp
            memo[i][j] = minim
    return minim
